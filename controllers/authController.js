...
exports.login = catchAsync(async (req, res, next) => {
  ...
  //   return next(
  //     new AppError(
  //       'User not verfied email address. Please verify an verification email sent to your email',
  //       401
  //     )
  //   );
  // }
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (currentUser.changedPassworddAfter(decoded.iat)) {
    return next(
...
exports.resendVerifyEmail = catchAsync(async (req, res, next) => {
  ...
  // 2) Generate the random reset token
  const verifyToken = user.createUserVerifyEmailToken();
  await user.save({ validateBeforeSave: false });
  ...
  try {
    ...
    res.status(200).json({
      status: 'success',
      message: 'verify email Token send to email!'
    });
  } catch (err) {
...
exports.VerifyEmail = async (req, res, next) => {
  ...
  const user = await User.findOne({
    ...
    userVerifyTokenExpires: { $gt: Date.now() }
  });
